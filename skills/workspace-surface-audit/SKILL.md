...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value harness-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
origin: harness
workloads: [core]
---
...
- Comparing official marketplace plugins against harness-native coverage
...
- Prefer harness-native workflows over generic "install another plugin" advice when harness can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in the harness
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent harness workspaces
...
- existing harness skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but harness lacks a billing-operator skill"
- "Google Drive is connected, but there is no harness-native Google Workspace operator workflow"
...
2. whether harness already has parity
3. whether harness only has primitives
4. whether harness is missing the workflow entirely

### Phase 3: Turn Gaps Into harness Decisions

For every real gap, recommend the correct harness-native shape:

| Gap Type | Preferred harness Shape |
|----------|---------------------|
...
2. **Parity**
   - where harness already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but harness lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete harness-native additions, ordered by impact
...
- If harness already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what harness should own next.
