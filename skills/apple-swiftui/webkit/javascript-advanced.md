...
```swift
// FAIL: Injection risk and hard to debug
let userInput = "'; document.cookie; '"
try await page.callJavaScript("alert('\(userInput)')")

// PASS: Named arguments are escaped and type-safe
try await page.callJavaScript(
    "alert(message)",
...
```swift
// FAIL: Page not ready -- script will fail or return unexpected results
page.load(URLRequest(url: someURL))
let title = try await page.callJavaScript("document.title")

// PASS: Wait for .finished navigation event before executing scripts
...
```swift
// FAIL: Handler must be set on configuration BEFORE creating the page
let page = WebPage()
// Too late to register a scheme handler

// PASS: Register on configuration first, then create page
var configuration = WebPage.Configuration()
...
```swift
// FAIL: Page scripts can see and interfere with your variables
try await page.callJavaScript(
    "var apiKey = key",
...
// PASS: Use an isolated world for sensitive operations
