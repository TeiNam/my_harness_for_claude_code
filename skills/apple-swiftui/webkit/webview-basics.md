...
```swift
// FAIL: No way to reload, go back, or detect loading state
...
// PASS: Full control through WebPage
...
```swift
// FAIL: WebView is in SwiftUI, but WebPage and configuration types require WebKit
import SwiftUI
// Missing: import WebKit

// PASS: Always import both
import SwiftUI
import WebKit
```
...
```swift
// FAIL: JavaScript may fail if page hasn't loaded
page.load(URLRequest(url: someURL))
try await page.callJavaScript("document.title")  // Page not ready

// PASS: Wait for navigation event to reach .finished
page.load(URLRequest(url: someURL))
