...
```swift
// FAIL: Setting delegate to self (View is a struct, can't be a delegate)
struct MapView: UIViewRepresentable {
    func makeUIView(context: Context) -> MKMapView {
        let mapView = MKMapView()
...
```swift
// FAIL: Putting NavigationStack inside UINavigationController (double navigation bar)
// UIKit side:
...
// PASS: If UIKit provides navigation, don't add NavigationStack
...
```swift
// FAIL: Trying to access UIKit view hierarchy from SwiftUI
struct MyView: View {
    var body: some View {
        Text("Hello")
            .onAppear {
...
// PASS: Use UIViewRepresentable if you need UIKit access
```

```swift
// FAIL: Migrating everything at once in a big-bang rewrite
// This almost always fails for non-trivial apps

// PASS: Migrate incrementally, one screen at a time
```
