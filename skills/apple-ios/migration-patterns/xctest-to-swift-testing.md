...
```swift
// FAIL: Using XCTest assertions in Swift Testing
import Testing
...
```swift
// FAIL: Prefixing test functions with "test" (unnecessary but harmless)
@Test
func testAddition() { }  // Works but the "test" prefix is redundant

// PASS: Clean function name
@Test
func addition() { }

// PASS: Or use a display name
@Test("Addition of positive integers")
func addition() { }
```

```swift
// FAIL: Using XCTestExpectation for async tests
@Test
func fetchData() {
    // Cannot use XCTestExpectation in Swift Testing
}

// PASS: Use async/await
@Test
...
```swift
// FAIL: Using class with var properties for test state
@Suite
class MyTests {
    var counter = 0
...
// PASS: Use struct -- each test gets its own instance
@Suite
struct MyTests {
    var counter = 0
...
```swift
// FAIL: Mixing XCTest and Swift Testing in the same type
class MyTests: XCTestCase {
    @Test  // Cannot use @Test on XCTestCase methods
    func something() { }
}

// PASS: Keep them in separate types (can be in the same file or different files)
class LegacyTests: XCTestCase {
