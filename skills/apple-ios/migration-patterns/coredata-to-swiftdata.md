...
```swift
// FAIL: Forgetting to provide init for @Model class
@Model
class Item {
    var title: String
    var timestamp: Date
...
```swift
// FAIL: Using NSPredicate format strings with @Query
@Query(filter: NSPredicate(format: "isComplete == NO"))  // Wrong type
private var items: [Item]

// PASS: Use #Predicate macro
@Query(filter: #Predicate<Item> { !$0.isComplete })
private var items: [Item]
```

```swift
// FAIL: Creating model objects with context parameter (CoreData habit)
let item = Item(context: modelContext)

// PASS: Create normally, then insert
let item = Item(title: "New")
modelContext.insert(item)
```

```swift
// FAIL: Using NSSet for relationships
@Model
class Item {
    var tags: NSSet?  // Wrong, use Swift types
}

// PASS: Use Swift arrays
@Model
class Item {
    var tags: [Tag]
}
