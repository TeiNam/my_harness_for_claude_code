...
```swift
// FAIL: Not calling transaction.finish() (transaction stays pending, may re-deliver)
let result = try await product.purchase()
...
// PASS: Always finish verified transactions
if case .success(let verification) = result {
    let transaction = try checkVerified(verification)
    unlockContent(for: transaction.productID)
...
```swift
// FAIL: Not listening for Transaction.updates at app startup
struct MyApp: App {
    var body: some Scene {
        WindowGroup {
            ContentView()
...
// PASS: Start listening at app launch
struct MyApp: App {
    @State private var store = Store()
...
```swift
// FAIL: Ignoring unverified transactions silently
for await result in Transaction.currentEntitlements {
...
// PASS: Log unverified transactions for debugging
for await result in Transaction.currentEntitlements {
    switch result {
...
```swift
// FAIL: Using restoreCompletedTransactions() with StoreKit 2
SKPaymentQueue.default().restoreCompletedTransactions()  // StoreKit 1 API

// PASS: Use Transaction.currentEntitlements or AppStore.sync()
try await AppStore.sync()
await updateEntitlements()
```

```swift
// FAIL: Assuming StoreKit config file behavior matches production exactly
// Local testing does not validate with App Store servers
// Subscription timing is accelerated (1 hour = a few minutes)

// PASS: Also test in sandbox environment before release
