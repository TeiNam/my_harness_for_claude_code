...
```swift
// FAIL: Swift class not inheriting from NSObject (invisible to ObjC)
@objc class MyHelper {
    @objc func doWork() { }
}
// Error: only classes that inherit from NSObject can be @objc

// PASS: Inherit from NSObject for ObjC visibility
...
```swift
// FAIL: Importing -Swift.h in a .h file (circular import)
// MyClass.h
#import "ProjectName-Swift.h"  // Causes circular dependency

// PASS: Use forward declaration in .h, import in .m
// MyClass.h
...
```swift
// FAIL: Assuming Swift structs/enums are visible to ObjC
@objc struct Point {  // Error: structs cannot be @objc
    var x: Double
    var y: Double
}

// PASS: Use class inheriting from NSObject, or keep as Swift-only type
...
```swift
// FAIL: Using Swift-only types in @objc methods
...
// PASS: Use ObjC-compatible types at the boundary
@objc class DataService: NSObject {
...
```swift
// FAIL: Migrating everything at once (big-bang rewrite)
// This almost always fails for non-trivial apps

// PASS: Migrate one file at a time, test after each migration
