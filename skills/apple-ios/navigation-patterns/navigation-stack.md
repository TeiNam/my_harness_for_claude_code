...
```swift
// PASS: Modern — type-safe, works with programmatic navigation
NavigationLink(value: item) {
...
```swift
// FAIL: Legacy — no programmatic navigation support, eager view creation
...
```swift
// FAIL: Double navigation bar, broken back button
NavigationStack {
    NavigationStack {
        Text("Hello")
    }
...
// FAIL: NavigationStack inside a pushed view
...
// PASS: One NavigationStack at the root, pushed views are plain
struct DetailView: View {
    var body: some View {
        Text("Detail")
...
```swift
// FAIL: NavigationLink pushes a value but no destination registered — tap does nothing
NavigationStack {
...
```swift
// FAIL: Destination registered outside the stack — never matched
VStack {
    NavigationStack {
        NavigationLink(value: item) { Text(item.name) }
    }
...
// PASS: Register inside the stack
NavigationStack {
    NavigationLink(value: item) { Text(item.name) }
        .navigationDestination(for: Item.self) { item in
            DetailView(item: item)
...
```swift
// FAIL: Two registrations for same type — undefined behavior
NavigationStack {
    content
...
// PASS: One registration per type, use an enum for multiple routes
enum Route: Hashable {
    case viewA(Item)
    case viewB(Item)
}
