...
```swift
// FAIL: Navigation pushes replace the entire tab bar
NavigationStack {
    TabView {
        HomeView()
...
// PASS: NavigationStack inside each tab
TabView {
    NavigationStack {
        HomeView()
    }
...
```swift
// FAIL: Don't mix old and new APIs
TabView {
    Tab("Home", systemImage: "house", value: .home) {
        HomeView()
    }
...
```swift
// FAIL: More than 5 tabs on iPhone — cluttered, tiny targets
TabView {
    Tab("Home", ...) { }
    Tab("Search", ...) { }
    Tab("Favorites", ...) { }
...
// PASS: Use TabSection with sidebarAdaptable for many sections
...
```swift
// FAIL: No way to programmatically switch tabs
TabView {
    Tab("Home", systemImage: "house") { HomeView() }
}

// PASS: Use selection binding for programmatic control
@State private var selectedTab: AppTab = .home
