...
```swift
// PASS: Single namespace, unique IDs per item
@Namespace private var namespace
...
// PASS: IDs must be unique across all sections in the same namespace
Section("Recent") {
...
```swift
// FAIL: matchedGeometryEffect does NOT work for NavigationStack transitions
NavigationLink(value: item) {
    ItemRow(item: item)
        .matchedGeometryEffect(id: item.id, in: namespace)  // Wrong API
}

// PASS: Use matchedTransitionSource for navigation
NavigationLink(value: item) {
    ItemRow(item: item)
        .matchedTransitionSource(id: item.id, in: namespace)
}
...
```swift
// FAIL: Source and destination use different IDs — no animation
.matchedTransitionSource(id: item.id, in: namespace)
// ...
.navigationTransition(.zoom(sourceID: "item-\(item.id)", in: namespace))

// PASS: Same ID value in both
.matchedTransitionSource(id: item.id, in: namespace)
// ...
.navigationTransition(.zoom(sourceID: item.id, in: namespace))
```
...
```swift
// FAIL: Source and destination use different namespaces
...
// PASS: Same namespace for source and destination
@Namespace private var namespace
...
```swift
// FAIL: Namespace in child view — destroyed when child is popped
...
// PASS: Namespace in the view that owns the NavigationStack or its direct children
