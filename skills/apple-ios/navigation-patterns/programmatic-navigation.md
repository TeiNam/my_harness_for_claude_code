...
```swift
// PASS: Works — conforms to both Hashable and Codable
struct Item: Hashable, Codable {
    let id: UUID
    let name: String
}

// FAIL: Won't compile for state restoration — missing Codable
...
```swift
// FAIL: Causes "Modifying state during view update" warning
var body: some View {
    if someCondition {
...
// PASS: Mutate in response to actions or onChange
...
```swift
// FAIL: Deep link pushes on top of existing stack — confusing
func handle(deepLink: DeepLink) {
    path.append(deepLink.destination)
}

// PASS: Clear stack first, then navigate
func handle(deepLink: DeepLink) {
...
```swift
// FAIL: Path doesn't trigger view updates when not @State
struct ContentView: View {
    let path = NavigationPath()
}

// PASS: Path must be @State (or in an @Observable class used via @State)
@State private var path = NavigationPath()

// PASS: Or in an @Observable coordinator injected as @State
@State private var coordinator = NavigationCoordinator()
NavigationStack(path: $coordinator.path) { }
```
