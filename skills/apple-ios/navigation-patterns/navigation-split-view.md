...
```swift
// FAIL: Unnecessary NavigationStack — detail has no drill-down
NavigationSplitView {
    SidebarView()
} detail: {
    NavigationStack {
...
// PASS: Just show the detail view directly
NavigationSplitView {
    SidebarView()
} detail: {
...
```swift
// FAIL: Double navigation bars, broken layout
NavigationStack {
    NavigationSplitView {
        SidebarView()
    } detail: {
...
// PASS: NavigationSplitView is a top-level container
NavigationSplitView {
    SidebarView()
} detail: {
    DetailView()
...
```swift
// FAIL: Force unwrap or ignoring nil selection
NavigationSplitView {
    SidebarView()
} detail: {
...
// PASS: Always provide a placeholder
NavigationSplitView {
    SidebarView()
} detail: {
    if let item = selectedItem {
...
```swift
// FAIL: Detail view keeps stale state when selection changes
} detail: {
    if let item = selectedItem {
