...
```swift
// PASS: Automatically scales with Dynamic Type
Text("Title")
    .font(.headline)
...
// PASS: Good - Scales with Dynamic Type
Text("Custom Title")
    .font(.custom("SF Pro Display", size: 20, relativeTo: .headline))

// PASS: Alternative approach
Text("Custom Title")
...
```swift
// FAIL: Bad - Layout breaks with large text
HStack {
...
```swift
// PASS: Limit scaling range when necessary
Text("Icon Label")
    .font(.caption)
    .dynamicTypeSize(.small ... .large)

// PASS: Good for fixed-size UI elements
Button {
    // action
} label: {
...
```swift
// PASS: Use semibold for emphasis
Text("Important Message")
    .font(.body.weight(.semibold))

// PASS: Use bold for strong emphasis
Text("Warning")
    .font(.body.bold())

// FAIL: Avoid overuse of heavy weights
Text("Regular text")
...
```swift
// PASS: Limit lines for truncation
Text("Long text here...")
    .lineLimit(2)
...
```swift
// FAIL: Bad - Inconsistent hierarchy
VStack {
...
// PASS: Good - Consistent hierarchy
VStack {
    Text("Header 1").font(.title)
...
```swift
// FAIL: Bad - Can overflow
HStack {
    Text("Very long expense name that might wrap")
    Text("$150")
}

// PASS: Good - Controlled truncation
HStack {
    Text("Very long expense name that might wrap")
        .lineLimit(1)
    Spacer()
