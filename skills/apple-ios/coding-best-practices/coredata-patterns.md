...
```swift
// FAIL: Creating new contexts
let context = NSManagedObjectContext(concurrencyType: .mainQueueConcurrencyType)

// FAIL: Multiple contexts without coordination
...
```swift
// PASS: Use shared context
let context = CoreDataStack.sharedInstance.managedObjectContext

// PASS: Background context for heavy operations
...
```swift
// FAIL: Saving without checking
try? context.save()

// FAIL: Saving in a loop
for user in users {
...
// FAIL: Fetching all data
let expenses = try? context.fetch(request)

// FAIL: String-based sorting
...
// PASS: Use predicates to filter
request.predicate = NSPredicate(format: "amount > %f", 100.0)

// PASS: Use sort descriptors
request.sortDescriptors = [
    NSSortDescriptor(keyPath: \Expense.date, ascending: false)
]

// PASS: Limit results if needed
request.fetchLimit = 50

// PASS: Fetch with error handling
do {
    let expenses = try context.fetch(request)
    return expenses
} catch {
...
```swift
// FAIL: Force unwrapping managed objects
let name = expense.name!
let amount = expense.amount!

// FAIL: Not checking for nil
...
// PASS: Guard for required properties
guard let name = expense.name, !name.isEmpty else {
    print("Expense has no name")
    return
}

// PASS: Provide defaults in Core Data model
...
```swift
// PASS: Fetch only what you need
request.propertiesToFetch = ["name", "amount"]
request.resultType = .dictionaryResultType

// PASS: Use fetch batching
