...
```swift
// PASS: Good: Only the core action
...
// FAIL: Bad: Exposing the full app with all tabs
struct AssistiveAccessContentView: View {
    var body: some View {
        TabView {
            MessagesTab()
...
```swift
// PASS: Good: Large, standard controls with generous spacing
VStack(spacing: 20) {
    Button("Call Mom") {
        placeCall(to: .mom)
    }
...
// FAIL: Bad: Small, densely packed custom controls
HStack(spacing: 4) {
...
```swift
// PASS: Good: Text and icon together
Button {
    startCamera()
} label: {
...
// FAIL: Bad: Icon only with no visible label
Button {
    startCamera()
} label: {
...
```swift
// PASS: Good: Linear step-by-step flow
NavigationStack {
    ChooseRecipientView()
        .navigationTitle("Send Message")
}

// FAIL: Bad: Multi-level nested navigation with side branches
NavigationSplitView {
    SidebarView()
} content: {
    CategoryView()
...
```swift
// PASS: Good: Confirmation before destructive action
...
// FAIL: Bad: Immediate destructive action with no confirmation
Button("Delete") {
    deletePhoto()
}
```
