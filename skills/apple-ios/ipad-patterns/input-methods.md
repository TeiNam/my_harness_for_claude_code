...
```swift
// FAIL: Wrong -- no shortcuts; keyboard users must tap screen
Button("New") { createNew() }
Button("Delete") { deleteSelected() }

// PASS: Right -- discoverable shortcuts for primary actions
Button("New") { createNew() }
    .keyboardShortcut("n", modifiers: .command)
Button("Delete") { deleteSelected() }
...
```swift
// FAIL: Wrong -- no visual feedback with trackpad/mouse
Button("Action") { doSomething() }

// PASS: Right -- pointer gets hover effect
Button("Action") { doSomething() }
    .hoverEffect()
```
...
```swift
// FAIL: Wrong -- both finger and pencil draw, user cannot scroll
canvasView.drawingPolicy = .anyInput

// PASS: Right -- pencil draws, finger scrolls
...
```swift
// FAIL: Wrong -- hardcoded double-tap behavior
func pencilInteractionDidTap(_ interaction: UIPencilInteraction) {
    toggleEraser()  // Ignores user preference
}

// PASS: Right -- check user's preferred action
func pencilInteractionDidTap(_ interaction: UIPencilInteraction) {
    switch UIPencilInteraction.preferredTapAction {
