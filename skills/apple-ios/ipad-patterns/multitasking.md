...
```swift
// FAIL: Wrong -- assuming full screen width
struct ContentView: View {
    var body: some View {
        HStack {
            sidebar
...
// PASS: Right -- use NavigationSplitView which handles resizing
struct ContentView: View {
    var body: some View {
        NavigationSplitView {
            SidebarView()
...
```swift
// FAIL: Wrong -- Info.plist has UIApplicationSupportsMultipleScenes = false
// App only ever has one window, Expose shows no multi-window option

// PASS: Right -- set to true in Info.plist
// <key>UIApplicationSupportsMultipleScenes</key>
// <true/>
```
...
```swift
// FAIL: Wrong -- global state shared across all windows
class AppDelegate: UIResponder, UIApplicationDelegate {
    var currentDocument: Document?  // Which window does this belong to?
}

// PASS: Right -- state lives in scene delegate or per-scene model
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
...
```swift
// FAIL: Wrong -- never cleaning up when scene disconnects
// Resources leak as system disconnects background scenes

// PASS: Right -- release heavy resources in sceneDidDisconnect
func sceneDidDisconnect(_ scene: UIScene) {
...
```swift
// FAIL: Wrong -- assumes full screen width
.frame(width: UIScreen.main.bounds.width)

// PASS: Right -- use GeometryReader or flexible layouts
GeometryReader { geometry in
    content
        .frame(width: geometry.size.width)
}

// PASS: Even better -- use flexible SwiftUI layout without explicit widths
HStack {
    sidebar
