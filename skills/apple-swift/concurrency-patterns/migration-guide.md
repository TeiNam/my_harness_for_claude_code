...
```swift
// FAIL: Swift 6 error: Static property is not concurrency-safe
class AppConfig {
...
```swift
// FAIL: Closure captures non-Sendable type
...
```swift
// FAIL: Swift 6 error: @MainActor type can't conform to non-isolated protocol
...
```swift
// PASS: Make the protocol method nonisolated
...
```swift
// FAIL: Warning: Type from ObjC module is not Sendable
import CoreLocation

// PASS: Suppress warnings for imported module
...
```swift
// FAIL: Delegate callback crosses isolation
...
```swift
// FAIL: Closure may run on any thread
...
// PASS: Specify main queue, or bridge to AsyncSequence
NotificationCenter.default.addObserver(
...
// PASS: Or use async notification stream
