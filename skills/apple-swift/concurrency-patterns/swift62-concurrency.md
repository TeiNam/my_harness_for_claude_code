...
```swift
// PASS: Swift 6.2 — No error. Async functions stay on the caller's actor.
...
```swift
// FAIL: Before (Swift 6.0/6.1) — manual annotations everywhere
@MainActor
...
```swift
// PASS: After (Swift 6.2 with infer main actor) — no annotations needed
final class StickerLibrary {
...
// FAIL: Swift 6.0/6.1 — ERROR: Conformance crosses into main actor-isolated code
