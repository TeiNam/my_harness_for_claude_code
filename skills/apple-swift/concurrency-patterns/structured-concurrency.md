...
```swift
// PASS: async let — different return types, fixed count
async let user = fetchUser()        // -> User
async let settings = fetchSettings() // -> Settings

// PASS: TaskGroup — same return type, dynamic count
...
```swift
// PASS: Discarding — good for side effects (logging, notifications, cache warming)
...
// FAIL: Don't use regular TaskGroup if you ignore results — leaks memory
...
```swift
// FAIL: Manual pattern — error-prone, must handle cancellation yourself
...
// PASS: .task(id:) handles cancellation automatically
.task(id: itemID) {
    item = await fetchItem(id: itemID)
}
```
...
```swift
// FAIL: Unstructured task — not cancelled when view disappears
.onAppear {
    Task {
...
// PASS: Structured — automatically cancelled on disappear
.task {
    items = await fetchItems()
}
```
...
```swift
// FAIL: Unstructured — no automatic cancellation, harder to reason about
func loadData() {
...
// PASS: Structured — automatic cancellation, clear lifetime
func loadData() async {
...
```swift
// FAIL: Memory leak — completed task values accumulate
...
// PASS: Always iterate, or use withDiscardingTaskGroup for Void results
await withTaskGroup(of: Data.self) { group in
    for url in urls {
        group.addTask { await download(url) }
    }
...
```swift
// FAIL: Runs to completion even when parent is cancelled
