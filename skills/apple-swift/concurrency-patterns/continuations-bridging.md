...
```swift
// FAIL: Bug — continuation never resumed on timeout
...
// FAIL: Bug — continuation resumed twice
...
```swift
// PASS: Ensure exactly one resume
func fetchData() async throws -> Data {
    try await withCheckedThrowingContinuation { continuation in
...
```swift
// FAIL: Retain cycle — continuation holds self, self holds continuation
...
// PASS: Use weak self in onTermination, nil out continuation
continuation.onTermination = { @Sendable [weak self] _ in
...
```swift
// FAIL: Overkill — AsyncStream for a one-shot result
...
// PASS: Use withCheckedContinuation for single values
