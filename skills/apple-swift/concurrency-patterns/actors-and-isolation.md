...
```swift
// FAIL: Bug — actor reentrancy
actor BankAccount {
    var balance: Int = 1000

    func withdraw(amount: Int) async -> Bool {
        guard balance >= amount else { return false }
        // WARNING: SUSPENSION POINT — another caller can run here
...
```swift
// PASS: Capture state before await
actor BankAccount {
    var balance: Int = 1000
...
```swift
// PASS: No suspension point in the critical path
actor BankAccount {
    var balance: Int = 1000
...
```swift
// PASS: Value type with Sendable properties — automatically Sendable
...
```swift
// WARNING: Use only when you can prove thread safety
...
```swift
// FAIL: NSMutableArray is not Sendable
actor Processor {
    func process(_ array: NSMutableArray) { }  // Compiler error
}

// PASS: Convert to Sendable type at the boundary
actor Processor {
...
```swift
// FAIL: Wrong mental model — actor is not a lock, it's a mailbox
actor Counter {
    var count = 0
...
// PASS: Correct mental model — avoid await between state mutations
actor Counter {
    var count = 0
...
```swift
// FAIL: Dangerous — silences compiler but doesn't fix the data race
class UnsafeCache: @unchecked Sendable {
    var items: [String: Any] = [:]  // No synchronization!
}

// PASS: Use an actor instead
actor SafeCache {
