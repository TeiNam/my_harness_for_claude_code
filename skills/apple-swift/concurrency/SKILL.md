...
```swift
// FAIL: Swift 6.0/6.1 -- ERROR: Sending 'self.processor' risks causing data races
@MainActor
...
```swift
// PASS: Swift 6.2 -- No error. extractSticker stays on the caller's actor.
@MainActor
final class StickerModel {
    let processor = PhotoProcessor()
...
```swift
// FAIL: Before (Swift 6.0/6.1) -- manual @MainActor annotations everywhere
@MainActor
...
```swift
// PASS: After (Swift 6.2 with default MainActor inference) -- no annotations needed
final class StickerLibrary {
...
```swift
// FAIL: Before -- required explicit annotation or Sendable conformance
@MainActor static let shared: StickerLibrary = .init()

// PASS: After -- default MainActor inference handles it
static let shared: StickerLibrary = .init()  // Implicitly @MainActor
```
...
// FAIL: Swift 6.0/6.1 -- ERROR: Main actor-isolated conformance crosses isolation boundary
...
```swift
// PASS: Swift 6.2 -- Isolated conformance
...
```swift
// PASS: Used within @MainActor context -- OK
@MainActor
...
// FAIL: Used outside @MainActor -- compile error
...
```swift
// FAIL: Wrong -- trivial work does not need @concurrent
...
// PASS: Right -- leave it on the calling actor
...
- Apple: Swift Concurrency documentation updates
...
```swift
// FAIL: Wrong -- library imposes MainActor on all consumers
// Package.swift
.target(
...
```swift
// FAIL: Wrong -- unnecessary thread hop for trivial work
@concurrent
func greet(_ name: String) async -> String {
    "Hello, \(name)"
}

// PASS: Right -- no @concurrent needed
...
// FAIL: Wrong -- this CPU-intensive parser now runs on MainActor, blocking UI
...
// PASS: Right -- opt out of MainActor for background-suitable types
...
// FAIL: Wrong -- trying to use the conformance from a nonisolated context
...
// PASS: Right -- use the conformance from a matching isolation context
@MainActor
...
```swift
// FAIL: Wrong -- removed annotations but did NOT enable default MainActor inference
class ViewModel {  // No longer @MainActor -- state is unprotected
    var items: [Item] = []
    func load() async { ... }
}

// PASS: Right -- either keep annotations OR enable the build setting
...
- **General concurrency patterns** (actors, TaskGroup, AsyncSequence, Sendable, cancellation): `skills/apple-swift/concurrency-patterns`
- **Actors and isolation deep dive**: `skills/apple-swift/concurrency-patterns/actors-and-isolation.md`
- **Structured concurrency patterns**: `skills/apple-swift/concurrency-patterns/structured-concurrency.md`
- **Swift 6 migration guide**: `skills/apple-swift/concurrency-patterns/migration-guide.md`
- **Apple documentation**: Swift Concurrency documentation updates
