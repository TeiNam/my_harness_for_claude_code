...
```swift
// PASS: Good -- background intent for data work
var supportedModes: IntentModes { .background }

// PASS: Good -- immediate foreground for camera/AR features
var supportedModes: IntentModes { .foreground(.immediate) }

// PASS: Good -- dynamic for intents that might need UI
var supportedModes: IntentModes { .foreground(.dynamic) }

// FAIL: Wrong -- using openAppWhenRun instead of supportedModes (legacy approach)
...
```swift
// PASS: Good -- descriptive dialog, meaningful option labels
let chosen = try await requestChoice(
    between: options,
    dialog: "Which account should I transfer from?"
)

// FAIL: Wrong -- vague dialog, no context
let chosen = try await requestChoice(
    between: options,
    dialog: "Pick one"
)
...
```swift
// PASS: Good -- entity identifier matches actual displayed content
...
// FAIL: Wrong -- generic activity with no entity association
.userActivity("com.myapp.viewItem") { activity in
...
```swift
// PASS: Good -- package declares its dependencies
...
// FAIL: Wrong -- intents duplicated across targets instead of shared via package
