...
```swift
// FAIL: Wrong -- no title or description
...
// PASS: Correct -- static title is required
struct MyIntent: AppIntent {
...
```swift
// FAIL: Wrong -- entities updated but Spotlight not notified
func saveRecipe(_ recipe: Recipe) {
    database.save(recipe)
}

// PASS: Correct -- reindex after mutations
...
```swift
// FAIL: Wrong -- forces app to foreground for a simple toggle
...
// PASS: Correct -- runs silently in background
struct ToggleFavoriteIntent: AppIntent {
    static var title: LocalizedStringResource = "Toggle Favorite"
...
```swift
// FAIL: Wrong -- entity has no way to be queried
...
// PASS: Correct -- provides a query so Siri can resolve entities
struct NoteEntity: AppEntity {
    var id: String
    var title: String
...
```swift
// FAIL: Wrong -- indexing thousands of items at once blocks the main thread
func indexAll() async throws {
    let allItems = database.fetchAll()  // 50,000 items
    try await CSSearchableIndex.default().indexAppEntities()
}

// PASS: Correct -- batch index and run off main thread
func indexAll() async throws {
    try await CSSearchableIndex.default().indexAppEntities(
