...
#   {skill_dir}/scripts/prepare_monolith_input.py
# metrics.py is at:
#   {skill_dir}/references/metrics.py
# (하네스는 스킬 디렉터리 하나를 심링크로 설치하므로 저장소 루트가 아니라
#  스킬 디렉터리가 기준이다 — 상류 im-not-ai 의 {repo}/scripts 레이아웃과 다른 점.)
HERE = Path(__file__).resolve().parent
SKILL_DIR = HERE.parent
METRICS_DIR = SKILL_DIR / "references"
...
# 주의: 한국어 활용에서 어근이 축약된다 — "쥘"(쥐+ㄹ)은 "쥐"와 다른 음절이라 어근만 매칭하면
