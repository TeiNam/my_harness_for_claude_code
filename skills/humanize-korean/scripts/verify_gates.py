...
# 하네스 레이아웃: {skill_dir}/scripts/ 와 {skill_dir}/references/ 가 형제다.
...
# 주의: 완곡 사전과 같은 이유로 넓게 잡는다. 구 사전은 "~야 한다" 계열과 "필요가 있다"뿐이라
...
# 주의: 완곡 사전은 넓어야 한다. 구 사전은 네 갈래뿐이라 **실측에서 완곡 7개 중 1개만 셌다** —
...
HEDGE_RE = _re.compile(
    ...
    r"|듯하|듯\s*싶"
    r"|것\s*같다"
    # 주의: "전망"·"예상"을 맨몸으로 넣으면 **"이전 전망치보다"** 같은 명사에 걸린다.
