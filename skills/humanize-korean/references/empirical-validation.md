...
|---|---|---|---|---|
| **장문 결핍**(100자+ 문장) | 8.1 | 91.3 | 60.9 **** | PASS: 인접 문장 잇기(내용 추가 금지). E-1의 실체 = "균일"이 아니라 "장문 부재" |
| **인용 결핍**(직접 인용부호) | 0.0 | 8.7 | 96.4 **** | FAIL: 관측만 — 없는 인용을 지어내면 안 됨 |
| **괄호 결핍** | 1.2 | 10.6 | 69.5 **** | FAIL: 관측만 |
...
WARNING: **측정 오류 정정**: 최초 I-1 측정에서 종결어미 마지막 2자(`이다`)를 세어 모든 `~이다` 종결이 섞였다(당시 AI 137.0 vs 인간 142.1, G²=0.1로 보고). `~것이다`만 정밀 측정하면 AI 20.4 vs 인간 43.0. 결론(기각) 방향은 같으나 근거 수치를 정정한다.
