...
## 스크립트 경로 규칙 (`${CLAUDE_SKILL_DIR}`)

**스크립트는 절대경로로 부른다. cwd 기준 상대경로로 부르면 안 된다.**

이 하네스는 스킬 디렉터리 하나를 `~/.claude/skills/humanize-korean` 심링크로 설치하고, `scripts/` 와 `references/` 를 그 안에 함께 둔다. 그래서 기준점이 하나다 — **`${CLAUDE_SKILL_DIR}`**. (상류 im-not-ai 는 `{repo}/scripts` + `{repo}/skills/...` 레이아웃이라 `.claude-plugin` 을 찾아 올라가는 `SKILL_ROOT` 유도가 필요했다. 하네스에는 플러그인 루트가 없으므로 그 단계를 뺐다.)

반면 `_workspace/` 같은 데이터 경로는 **cwd 기준**이다(run_id 규칙 참조). 한 명령줄에 두 기준이 섞이므로 스크립트·룰북 쪽만 절대경로로 고정한다. 룰북·taxonomy 경로도 접두어 없이 쓰면 cwd 로 풀려 `No such file or directory` 가 난다.

**확인**: Phase 1 시작 전에 `ls "${CLAUDE_SKILL_DIR}/scripts/prepare_monolith_input.py"` 를 한 번 돌린다. 실패하면 `${CLAUDE_SKILL_DIR}` 가 비었거나 설치가 깨진 것이다 — 스크립트를 찾을 때까지 임의로 추측하지 말고, `~/.claude/skills/humanize-korean/scripts/` 를 한 번 확인한 뒤 그것도 없으면 정량 shim·게이트 없이 진행한다고 **사용자에게 알린 뒤** 계속한다. 조용히 건너뛰면 route_hint 와 철칙 #4 게이트가 사라진 것을 아무도 모른다.
...
4. 사전 처리 shim을 Bash로 1회 실행:
   ```
   python3 ${CLAUDE_SKILL_DIR}/scripts/prepare_monolith_input.py --run-dir _workspace/{run_id} --genre {genre}
   ```
...
2. shim으로 진단을 monolith 입력 앞에 결합 (Bash — LLM 콜 아님):
   ```
   python3 ${CLAUDE_SKILL_DIR}/scripts/prepare_monolith_input.py --run-dir _workspace/{run_id} --genre {genre} --diagnosis _workspace/{run_id}/02_diagnosis.md
   ```
...
1. shim으로 진단 결합 (Bash). **heavy에서만** `--chunk`를 함께 줄 수 있다:
   ```
   python3 ${CLAUDE_SKILL_DIR}/scripts/prepare_monolith_input.py --run-dir _workspace/{run_id} --genre {genre} --diagnosis _workspace/{run_id}/02_diagnosis.md --chunk
   ```
...
4. **청크 병렬(shim이 실제로 쪼갠 경우만)**:
   ...
   - 재조립: `python3 ${CLAUDE_SKILL_DIR}/scripts/reassemble_chunks.py --run-dir _workspace/{run_id}` → `03_reassembled.md`(passthrough 원문 삽입 + 문자수 대사). 이걸 `final.md`로 삼는다.
...
```
python3 ${CLAUDE_SKILL_DIR}/scripts/restore_modality.py \
    --before _workspace/{run_id}/01_input.txt \
    --after  _workspace/{run_id}/final.md \
    --out    _workspace/{run_id}/final.md
python3 ${CLAUDE_SKILL_DIR}/scripts/strip_injected_commas.py \
    --before _workspace/{run_id}/01_input.txt \
    --after  _workspace/{run_id}/final.md \
    --out    _workspace/{run_id}/final.md
```
...
```
python3 ${CLAUDE_SKILL_DIR}/scripts/verify_gates.py \
    --before _workspace/{run_id}/01_input.txt \
    --after  _workspace/{run_id}/final.md \
    --genre {genre}
```
...
**모델:** 런타임 3종 모두 `model: opus`. 세 콜이 모두 **의미 보존 판정**을 들고 있어서다 — 내용 앵커 원형 보존(monolith), 무엇을 겨냥할지의 판단(diagnostician), 의미 보존 15항(finalizer). 티어를 내리면 가장 먼저 새는 것이 "AI 관용구를 걷다가 주장까지 함축해버리는" 실수이고, 그 미스는 결정적 게이트로도 전부 못 잡는다(게이트는 표지·대구·수치만 본다). 더 깊은 판단이 필요하면 티어가 아니라 `effort:`를 올린다.

**에이전트 정의 위치:** 하네스 `agents/`에 10종 정의. `install.sh`(또는 `install.ps1`)가 `agents/*.md`를 `~/.claude/agents/`에 심링크한다.

10종의 내역은 런타임 3 + 유지보수 1 + 웹 확장 설계 1 + 개발용 1회성 5이며, **본 스킬 런타임이 호출하는 것은 3종뿐**이다.
...
- `korean-ai-tell-taxonomist` — 분류 체계(SSOT) 유지·확장. 본 스킬 실행 중에는 호출되지 않음. taxonomy를 고친 뒤에는 `scripts/build_quick_rules.py`·`scripts/build_diagnosis_rules.py`를 돌려 생성물(`quick-rules.md`·`diagnosis-rules.md`)을 재생성한다 — 손 동기화 금지

**웹 확장 1종 (하네스 로컬, 런타임 호출 없음)**
- `humanize-web-architect` — `references/web-service-spec.md` 기반 웹 서비스 설계용

(개발용 1회성 5종·v2.1 은퇴 5종의 계보와 테스트 시나리오는 `${CLAUDE_SKILL_DIR}/references/design-notes.md` 참조. 은퇴 5종 중 `ai-tell-detector`·`korean-style-rewriter`·`naturalness-reviewer`·`content-fidelity-auditor` 4종은 v2.3 동기화 때 하네스에서도 삭제했다 — 상시 로드되는 description 예산만 먹고 호출되지 않았다.)
...
- 정량 점수 shim: `${CLAUDE_SKILL_DIR}/scripts/prepare_monolith_input.py` — `${CLAUDE_SKILL_DIR}/references/metrics_v2.py`(실패 시 `metrics.py` fallback) + `${CLAUDE_SKILL_DIR}/references/baseline.json` 기반 사전 점수 + `route_hint` 산출
- 텍스트 위생: `${CLAUDE_SKILL_DIR}/scripts/sanitize_text.py` — shim이 자동 호출(끄려면 `--no-sanitize`). 제로폭·bidi·특수공백 제거 + 한글 NFD→NFC 정규화를 `01_input.txt`에 반영해 이후 변경률 게이트·diff·글자수가 같은 기준을 쓰게 한다. 결정적 처리, LLM 0콜. 변경이 있으면 `00_sanitize.json` 기록. **AI 워터마크 제거 기능이 아니다** (근거는 `scripts/sanitize_text.py` 상단 주석)
