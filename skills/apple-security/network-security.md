...
### WARNING: Exception: Specific Domain Needs HTTP
...
### FAIL: Never Do This in Production
...
print("Auth token: \(token)")  // FAIL:
print("Request body: \(requestBody)")  // FAIL:
print("Response: \(responseData)")  // FAIL:

// OK: Log non-sensitive metadata
print("Request to: \(url.host ?? "unknown")")  // PASS:
print("Status code: \(statusCode)")  // PASS:
```
...
let apiKey = "sk-ant-api03-xxxxx"  // FAIL:

// Store in Keychain or use environment/config
let apiKey = try KeychainManager.shared.readString(account: "api_key")  // PASS:
```
