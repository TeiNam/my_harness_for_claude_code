...
- FAIL: Can't be pronounced correctly on first read
- FAIL: Can't be spelled correctly after hearing it once (the "radio test")
- FAIL: (iOS) Doesn't read cleanly when truncated to ~12 characters on the Home Screen — relaxes for Mac apps (see naming-strategies.md)
- FAIL: Confusable with a major existing app (Notes, Notion, Things, etc.)
- FAIL: Purely generic/descriptive with no distinctiveness (un-trademarkable)
- FAIL: Produces an ugly bundle ID or has awkward casing
