...
```swift
// PASS: GOOD: Sendable conformance
struct User: Sendable {
    let id: UUID
    let name: String
}

// FAIL: BAD: Mutable reference type without protection
class UserManager {
    var users: [User] = []  // Not thread-safe!
}

// PASS: GOOD: Use actor for mutable state
actor UserManager {
...
### Macro System
```swift
// PASS: Use macros for reducing boilerplate
import SwiftData
...
```swift
// PASS: GOOD: Specific error types
...
```swift
// PASS: GOOD: Reference type for identity and shared state
...
// PASS: GOOD: Reference type for inheritance
...
```swift
// PASS: GOOD: Small, focused protocols
...
### Protocol Extensions
```swift
// PASS: GOOD: Default implementations
...
```swift
// FAIL: BAD: Runtime type checking
...
// PASS: GOOD: Protocol-based approach
...
### Generic Functions
```swift
// PASS: GOOD: Generic function with constraints
...
### Associated Types
```swift
// PASS: GOOD: Protocol with associated type
protocol Repository {
...
### Optional Binding
```swift
// PASS: GOOD: Guard for early exit
func processUser(_ user: User?) {
    guard let user else { return }
    // Work with unwrapped user
}

// PASS: GOOD: If-let for scoped usage
if let user = optionalUser {
    print(user.name)
}

// FAIL: BAD: Force unwrapping
let name = user!.name  // Dangerous!

// FAIL: BAD: Implicit unwrapping (use sparingly)
var user: User!
```

### Nil-Coalescing and Optional Chaining
```swift
// PASS: GOOD: Nil-coalescing with default
let displayName = user?.name ?? "Guest"

// PASS: GOOD: Optional chaining
let uppercasedName = user?.name?.uppercased()

// PASS: GOOD: Optional map and flatMap
...
```swift
// PASS: GOOD: Custom property wrapper for validation
@propertyWrapper
...
### SwiftUI-Style DSL
```swift
// PASS: GOOD: Result builder for custom DSL
@resultBuilder
struct MenuBuilder {
...
```swift
// PASS: GOOD: Well-structured error types
...
```swift
// PASS: GOOD: Array for ordered collections
var items: [Item] = []

// PASS: GOOD: Set for unique values and fast lookup
var uniqueIDs: Set<UUID> = []

// PASS: GOOD: Dictionary for key-value pairs
var usersByID: [UUID: User] = [:]

// PASS: GOOD: OrderedSet (Swift Collections) when order + uniqueness matter
...
// PASS: GOOD: CompactMap for removing nils
let validURLs = strings.compactMap { URL(string: $0) }

// PASS: GOOD: FlatMap for flattening nested collections
...
```swift
// PASS: GOOD: Weak self to avoid retain cycles
...
// PASS: GOOD: Unowned when guaranteed to exist
...
```swift
// FAIL: BAD: Strong reference cycle
class Author {
    var books: [Book] = []
}
...
// PASS: GOOD: Break cycle with weak reference
class Author {
    var books: [Book] = []
}
