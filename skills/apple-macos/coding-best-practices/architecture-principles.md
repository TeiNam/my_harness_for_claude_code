...
// PASS: GOOD: Separated responsibilities
...
```swift
// FAIL: BAD: Must modify class to add new export formats
...
// PASS: GOOD: Open for extension via protocols
...
```swift
// FAIL: BAD: Violates LSP - ReadOnlyDocument can't fulfill Document contract
class Document {
    var content: String
...
```swift
// FAIL: BAD: Fat protocol forcing unnecessary implementations
...
// PASS: GOOD: Segregated interfaces
protocol Playable {
    func play()
    func pause()
    func stop()
...
```swift
// FAIL: BAD: High-level class depends on concrete implementation
...
// PASS: GOOD: Depend on abstraction
...
### Code Duplication
```swift
// FAIL: BAD: Repeated validation logic
...
// PASS: GOOD: Extract common validation
...
```swift
// FAIL: BAD: Repeated UI components
...
```swift
// FAIL: BAD: Hardcoded values everywhere
...
// PASS: GOOD: Centralized configuration
enum APIEndpoint {
    case users
    case posts
    case articles
...
```swift
// PASS: GOOD: Dependencies injected via initializer
...
```swift
// WARNING: Use sparingly - hides dependencies
...
```swift
// FAIL: BAD: Deep inheritance hierarchy
class Vehicle {
    func start() { }
}
...
// PASS: GOOD: Composition with protocols
protocol Startable {
    func start()
}
