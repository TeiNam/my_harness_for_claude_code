...
```swift
// PASS: GOOD: Async computed property
class ImageLoader {
    var image: UIImage {
        get async throws {
...
// PASS: GOOD: Detached task (runs independently)
Task.detached {
...
```swift
// PASS: GOOD: Parallel fetching with task group
...
// PASS: GOOD: With error handling per task
...
```swift
// PASS: GOOD: Processing async sequences
...
// PASS: GOOD: Custom async sequence
...
```swift
// FAIL: BAD: Unsafe class with mutable state
class Counter {
...
```swift
// PASS: GOOD: MainActor for UI updates
@MainActor
...
```swift
// PASS: GOOD: Value types are automatically Sendable
struct User: Sendable {
    let id: UUID
    let name: String
}

// PASS: GOOD: Immutable classes can be Sendable
...
// FAIL: BAD: Mutable class can't be Sendable safely
final class Counter: Sendable {  // Warning!
    var count = 0  // Mutable property not thread-safe
}

// PASS: GOOD: Use actor instead
actor Counter {
    var count = 0  // Thread-safe via actor isolation
}

// PASS: GOOD: @unchecked Sendable when you know it's safe
...
// FAIL: BAD: Capturing mutable state
var counter = 0
processAsync {
...
```swift
// PASS: GOOD: Checking for cancellation
...
// PASS: GOOD: Cancellation in view model
@MainActor
...
// PASS: GOOD: Run computation off main thread
@MainActor
func loadData() async {
...
```swift
// FAIL: BAD: Unstructured tasks can leak
func fetchAllData() {
    Task {
...
// PASS: GOOD: Structured with task group
...
```swift
// PASS: GOOD: AsyncStream for real-time updates
