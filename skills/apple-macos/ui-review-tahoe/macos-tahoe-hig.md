...
```swift
// PASS: GOOD: Modern window with unified toolbar
@main
struct MyApp: App {
    var body: some Scene {
        WindowGroup {
...
// PASS: GOOD: Toolbar with standard controls
...
// FAIL: BAD: Custom window chrome (avoid unless necessary)
```

### Window Sizing and Constraints

```swift
// PASS: GOOD: Appropriate size constraints
WindowGroup {
    ContentView()
}
...
```swift
// PASS: GOOD: Supporting multiple windows
@main
struct MyApp: App {
    var body: some Scene {
        // Main window
...
```swift
// PASS: GOOD: Well-organized toolbar
.toolbar(id: "main") {
    // Leading items
...
```swift
// PASS: GOOD: Allow toolbar customization
...
```swift
// PASS: GOOD: Standard keyboard shortcuts
...
// WARNING: Don't override system shortcuts
// Avoid: ⌘Q, ⌘H, ⌘M, ⌘Tab, etc.

// PASS: GOOD: Function key shortcuts
...
```swift
// PASS: GOOD: TabView for distinct sections
TabView {
    DashboardView()
        .tabItem {
...
```swift
// PASS: GOOD: Appropriate button styles
...
```swift
// PASS: GOOD: List with context menu
...
```swift
// WARNING: Touch Bar removed on newer Macs
...
```swift
// PASS: GOOD: Make content searchable
...
```swift
// PASS: GOOD: Support phone links for Phone app integration
