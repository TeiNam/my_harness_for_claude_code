...
CONFIG_DIR="$(_harness_resolve_homunculus_dir)"
...
#   - harness observing its own Haiku observer sessions (self-loop)
#   - harness observing other tools' automated sessions
...
# (HARNESS_HOOK_PROFILE=minimal, HARNESS_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${HARNESS_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${HARNESS_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_HARNESS_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_HARNESS_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_HARNESS_SKIP_PATHS="${HARNESS_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _HARNESS_SKIP_ARRAY <<< "$_HARNESS_SKIP_PATHS"
  for _pattern in "${_HARNESS_SKIP_ARRAY[@]}"; do
...
SIGNAL_EVERY_N="${HARNESS_OBSERVER_SIGNAL_EVERY_N:-20}"
