...
**L-01 · No PASS:/FAIL: examples in prose.** The file has no `PASS:` or `FAIL:` markers anywhere.
- **Detection**: `Grep -L` for `PASS:|FAIL:`.
...
| L-01 (no PASS:/FAIL: examples) | Suppressed — aggregators don't carry patterns |
...
- Medium: <count>
- Low: <count>
- PASS: Files clean: <clean-count> / <N>
...
- `skills/apple-design/liquid-glass/SKILL.md:1-4` (leaf) — **Fix:** Add `allowed-tools: [Read, Glob, Grep]`.
- `skills/apple-macos/macos-tahoe-apis/SKILL.md:1-4` (leaf) — **Fix:** Same.
...
- `skills/apple-design/liquid-glass/SKILL.md` (leaf)
- `skills/apple-macos/coding-best-practices/SKILL.md` (leaf)
...
2. **Scoped run** — invoke with `apple-generators/`. Only `skills/apple-generators/**` paths appear.
3. **Single-file run** — invoke with `skills/apple-design/liquid-glass/SKILL.md`. Report has exactly one H-01 finding.
...
- Upstream `CLAUDE.md` (rshankras/claude-code-apple-skills) — normative source for frontmatter schema, naming, emoji convention
- `skills/apple-shared/skill-creator/SKILL.md` — companion meta-skill for creating new skills (this auditor only audits; it does not create)
- `skills/apple-ios/SKILL.md` — canonical aggregator example
