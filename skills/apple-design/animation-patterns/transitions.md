...
VStack {
    if showDetail {
        DetailView()
            .transition(.slide)  // PASS: Works — view is added/removed
    }
}
...
VStack {
    DetailView()
        ...
        .transition(.slide)             // FAIL: No effect — view is always present
}
```
...
```swift
// FAIL: WRONG: Both set isSource: true
view1.matchedGeometryEffect(id: "x", in: ns, isSource: true)
view2.matchedGeometryEffect(id: "x", in: ns, isSource: true)
// PASS: Only ONE should be isSource: true

// FAIL: WRONG: Forgetting @Namespace
let animation = Namespace()  // This is wrong
// PASS: @Namespace private var animation

// FAIL: WRONG: Using for NavigationStack push/pop transitions
...
// PASS: Use matchedTransitionSource (iOS 18+) instead — see below
```
