...
```swift
// FAIL: Deprecated in iOS 15 — animates ALL state changes, causes unexpected behavior
Circle()
    .animation(.spring())

// PASS: Always pass value:
Circle()
    .animation(.spring, value: isActive)
```
...
```swift
// FAIL: WRONG: Mixing generation 2 and 3 parameter names
.spring(response: 0.5, bounce: 0.3)   // Does NOT compile

// FAIL: WRONG: Mixing generation 1 and 2
.spring(mass: 1.0, dampingFraction: 0.7)  // Does NOT compile

// PASS: Pick ONE generation:
...
```swift
// FAIL: WRONG: This modifier does NOT exist
view.onAnimationCompleted(for: offset) { }

// FAIL: WRONG: This modifier does NOT exist
view.onAnimationEnd { }

// PASS: Use withAnimation completion:
