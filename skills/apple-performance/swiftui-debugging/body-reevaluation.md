...
```swift
// FAIL: Parent rebuilds child every time its own body runs
...
// PASS: Extract child so its body only runs when its own deps change
struct ParentView: View {
    @State private var counter = 0
...
struct MyView: View {
    ...
    // FAIL: This read happens in init, not body -- no observation established
    var label: String { "Value: \(model.value)" }

    var body: some View {
        // PASS: Must read model.value inside body (directly or via computed property called from body)
...
struct NameView: View {
    ...
    var body: some View {
        // PASS: This observes both firstName and lastName
...
```swift
// FAIL: Monolithic: entire view re-evaluates when any state changes
...
// PASS: Split: each section only re-evaluates for its own state
struct ProfileView: View {
    var user: User
...
```swift
// FAIL: State at the top causes the entire tree to re-evaluate
...
// PASS: Encapsulate search state in a dedicated view
struct ContentView: View {
    var body: some View {
        NavigationStack {
            VStack {
...
```swift
// FAIL: Reads colorScheme even though it does not use it
struct ItemRow: View {
    @Environment(\.colorScheme) private var colorScheme
    let item: Item
...
// PASS: Only read environment values you actually use in body
struct ItemRow: View {
    let item: Item
