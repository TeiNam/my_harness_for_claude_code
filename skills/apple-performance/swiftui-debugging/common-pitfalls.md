...
```swift
// FAIL: AnyView defeats SwiftUI's diffing optimization
...
```swift
// PASS: @ViewBuilder preserves type information for the compiler
@ViewBuilder
...
// PASS: Or better: dedicated view struct (avoids large switch in @ViewBuilder)
...
```swift
// FAIL: Creates a new DateFormatter every time body runs
...
```swift
// PASS: Static shared formatter -- created once, reused forever
...
// PASS: Or use the modern formatted() API which handles caching internally
struct DateLabel: View {
    let date: Date
...
```swift
// FAIL: All views observing AppState re-evaluate when ANY property changes
...
```swift
// PASS: Per-property observation -- views only update when their dependencies change
@Observable
class AppState {
    var user: User?
    var items: [Item] = []
...
```swift
// PASS: Each store has a narrow scope
...
```swift
// FAIL: Entire view re-evaluates when searchText changes
...
// PASS: Each subview only connects to what it needs
struct ContentView: View {
    @ObservedObject var store: AppState
...
```swift
// FAIL: Filtering and sorting 10,000 items every time body runs
var body: some View {
...
```swift
// PASS: Compute on change, not on every body call
...
```swift
// PASS: Background computation for expensive operations
struct ItemListView: View {
    let items: [Item]
    @State private var searchText = ""
    @State private var filteredItems: [Item] = []
...
```swift
// FAIL: Loads full resolution, decodes on main thread
...
```swift
// PASS: AsyncImage handles loading and caching
List(photos) { photo in
...
// PASS: For local images: thumbnail generation off main thread
...
```swift
// FAIL: Triggers re-evaluation even when value hasn't changed
.onReceive(timer) { _ in
    currentTime = Date()  // Always a new value, always triggers body
}

// FAIL: Setting state in body (causes infinite loop risk)
var body: some View {
...
```swift
// PASS: Guard against redundant updates
.onReceive(timer) { _ in
    let newTime = Date()
...
// PASS: Use TimelineView for time-based updates (iOS 15+)
...
```swift
// FAIL: Each modifier adds a wrapper view to the tree
Text("Hello")
    .padding()
...
```swift
// PASS: Flatten by using composite modifiers and fewer nesting levels
Text("Hello")
    .padding(8)
    .background(
...
```swift
// PASS: Encapsulate common modifier combinations
...
```swift
// FAIL: View is invisible but still evaluated and in the layout
ExpensiveView()
...
```swift
// PASS: View is not created at all when hidden
if showExpensive {
    ExpensiveView()
}

// PASS: Or use Group with optional for cleaner syntax
Group {
    if showExpensive {
        ExpensiveView()
    }
