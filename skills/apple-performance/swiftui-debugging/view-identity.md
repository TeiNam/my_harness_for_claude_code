...
```swift
// PASS: Stable: database ID, UUID, or model-provided unique key
ForEach(items, id: \.databaseID) { item in
    ItemRow(item: item)
}

// PASS: Stable: using Identifiable conformance
ForEach(items) { item in  // uses item.id automatically
    ItemRow(item: item)
}

// FAIL: Unstable: array index on a mutable array
...
// FAIL: Unstable: random value or Date()
...
```swift
// FAIL: Problem: toggling isLoading destroys and recreates ContentView
...
```swift
// FAIL: Bad: destroys and recreates the same Text view
if isHighlighted {
    Text(item.title)
...
// PASS: Good: same structural identity, just different modifier values
Text(item.title)
...
```swift
// PASS: Ternary: single structural identity, SwiftUI diffs the values
RoundedRectangle(cornerRadius: 12)
    .fill(isSelected ? Color.blue : Color.gray)
    .frame(height: isExpanded ? 200 : 80)

// FAIL: if/else: two different structural identities
if isSelected {
...
```swift
// FAIL: AnyView: SwiftUI cannot track structural identity across renders
...
// PASS: @ViewBuilder: preserves structural identity
@ViewBuilder
...
```swift
// PASS: Identifiable conformance (cleanest)
...
```swift
// FAIL: Dangerous: if two items have the same title, behavior is undefined
ForEach(items, id: \.title) { item in
    ItemRow(item: item)
}

// PASS: Always use a truly unique identifier
ForEach(items, id: \.id) { item in
    ItemRow(item: item)
}
```
