...
```swift
// FAIL: Wrong — data will be lost after 8 hours
UserDefaults.standard.set(orderID, forKey: "lastOrder")

// PASS: Right — persist in App Group for full app to access
...
```swift
// PASS: Show UI immediately, load data in background
...
// FAIL: Blank screen while loading
struct OrderExperienceView: View {
    var body: some View {
...
```swift
// PASS: Allow anonymous ordering, collect identity later
struct OrderFlow {
...
// FAIL: Block the experience with a login screen
struct OrderFlow {
    func placeOrder() {
...
```swift
// PASS: Show upgrade after task completion
...
```swift
// PASS: Only request location for physical-location experiences
// PASS: Use Sign in with Apple (minimal friction) if auth is needed
// PASS: Use Apple Pay (no form filling)

// FAIL: Don't request notification permission in App Clip
// FAIL: Don't request camera unless core to the experience
// FAIL: Don't request contacts, calendar, etc.
```
...
```swift
// FAIL: 5 MB image bundled in asset catalog
Image("hero-background") // Eats half the size budget

// PASS: Use SF Symbols or load from network
...
```swift
// FAIL: Adding Alamofire, SDWebImage, etc. bloats the binary
// Each SPM dependency can add 0.5-2 MB

// PASS: Use URLSession directly — it's already available
let (data, response) = try await URLSession.shared.data(from: url)
```

### Don't Persist Sensitive Data in App Clip Sandbox
```swift
// FAIL: Keychain data is deleted with App Clip data
try keychain.set(token, forKey: "authToken")

// PASS: Store in App Group if full app needs it
SharedDataManager.shared.save(token, forKey: "authToken")
```

### Don't Ignore the Size Budget During Development
```swift
// FAIL: "We'll optimize later" — then you're 15 MB at submission

// PASS: Check size on every PR
