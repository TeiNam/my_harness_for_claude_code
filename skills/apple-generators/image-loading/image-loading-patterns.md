...
|---------|-----------|-----------------|
| Memory cache | FAIL: None | PASS: NSCache |
| Disk cache | FAIL: None | PASS: LRU FileManager |
| Request deduplication | FAIL: Each view downloads independently | PASS: Single download shared |
| Prefetching | FAIL: Not possible | PASS: Preload before visible |
| Image processing | FAIL: Returns original size | PASS: Resize, thumbnail |
| Cancel on disappear | WARNING: Inconsistent | PASS: Explicit cancellation |
| Memory pressure handling | FAIL: No eviction | PASS: NSCache auto-evicts |
| Offline support | FAIL: None | PASS: Serve from disk cache |
| Cache key customization | FAIL: None | PASS: Custom keys |
...
|----------|----------|-----|
| 4000x3000 photo in a 44x44 avatar | PASS: Resize | Saves ~95% memory |
| Icon from CDN already sized | FAIL: | Already optimized |
| Photo gallery (full screen) | WARNING: Maybe | Resize to screen size, not original |
| Thumbnail in list | PASS: Thumbnail | Small size for fast scrolling |
...
```swift
// FAIL: Blocks UI while decoding large image
let image = UIImage(data: downloadedData)!
imageView.image = image

// PASS: Decode in background, display on main
Task.detached {
    let image = UIImage(data: downloadedData)
    // Force decode
...
```swift
// FAIL: 48 MB per image in cache
cache.setObject(fullSizeImage, forKey: url)

// PASS: Cache at display size
...
```swift
// FAIL: Memory cache grows unbounded
var cache: [URL: UIImage] = [:]  // Dictionary doesn't auto-evict

// PASS: NSCache auto-evicts under memory pressure
...
```swift
// FAIL: Downloads every time cell appears
.onAppear { Task { image = await download(url) } }

// PASS: Check cache first
