...
```swift
// PASS: Good: Descriptive names with clear defaults
enum FeatureFlag: String, CaseIterable, Sendable {
...
// FAIL: Bad: Vague names, no structure
...
```swift
// PASS: Good: Check via the manager (reactive, testable)
@Environment(FeatureFlagManager.self) private var flags
...
// FAIL: Bad: Read UserDefaults directly (not reactive, not testable)
...
```swift
// PASS: Good: Protocol-based, swappable providers
...
// FAIL: Bad: Hardcoded single implementation with no abstraction
class FeatureFlags {
...
```swift
// PASS: Good: Refresh on launch, handle errors gracefully
...
// FAIL: Bad: Block app launch on network call
func application(_ application: UIApplication,
                 didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
...
```swift
// PASS: Good: Remove flag after full rollout
...
// FAIL: Bad: Leave stale flags indefinitely
...
```swift
// PASS: Good: @MainActor manager, Sendable providers, NSLock for mutable state
@MainActor
@Observable
final class FeatureFlagManager {
...
// FAIL: Bad: Unprotected mutable state accessed from multiple threads
