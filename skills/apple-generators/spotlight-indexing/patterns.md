...
attributes.title = "Chocolate Chip Cookie Recipe"  // PASS: Specific, searchable

// DON'T: Use generic titles
attributes.title = "Recipe #42"  // FAIL: Not useful for search
...
```swift
// FAIL: Content deleted but still appears in Spotlight
func delete(_ item: Item) {
    database.delete(item)
    // Forgot to remove from Spotlight!
}

// PASS: Always pair delete with index removal
...
```swift
// FAIL: Full-size photo as thumbnail (10 MB)
attributes.thumbnailData = photo.fullSizeJPEGData

// PASS: Compressed, resized thumbnail (< 50 KB)
...
```swift
// FAIL: Swallowing errors completely
try? await index.indexSearchableItems(items)

// PASS: Log errors for diagnostics
do {
    try await index.indexSearchableItems(items)
} catch {
