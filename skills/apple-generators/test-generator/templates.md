...
```swift
// PASS: Good: Each test creates its own dependencies (Swift Testing)
@Suite("Tests")
struct MyTests {
    let sut: ItemViewModel
...
// FAIL: Bad: Shared mutable state without reset
@Suite("Tests")
struct MyTests {
...
```swift
// PASS: Good: Test specific error type (Swift Testing)
...
// FAIL: Bad: Only test that "some" error is thrown
@Test("throws error")
...
```swift
// PASS: Good: Use async/await directly (Swift Testing)
...
// FAIL: Bad: Using XCTest expectations for simple async (when you could use async/await)
...
```swift
// PASS: Good: Use #require to unwrap (Swift Testing)
...
// PASS: Good: Use XCTUnwrap (XCTest)
...
// FAIL: Bad: Force unwrap in tests
