...
| Use Case | SWR? | Why |
|----------|------|-----|
| User profile | PASS: | Data changes rarely, stale is fine |
| Feed/timeline | PASS: | Show last known, refresh in background |
| Shopping cart | FAIL: | Must be accurate |
| Payment status | FAIL: | Must be real-time |
| Config/settings | PASS: | Changes rarely, stale is fine briefly |
...
```swift
// FAIL: Never cache POST/PUT/DELETE responses
// PASS: Only cache GET (and optionally HEAD) requests
```

### Don't Cache Auth-Dependent Without User Key
```swift
// FAIL: Cache /api/me across users
// PASS: Include user ID in cache key for personalized endpoints
```

### Don't Ignore Cache-Control: no-store
```swift
// FAIL: Cache everything regardless of server headers
// PASS: Respect no-store — the server says this data must not be persisted
```

### Don't Use In-Memory Only
```swift
// FAIL: NSCache alone — cleared on memory pressure, lost on app restart
// PASS: Disk cache with memory layer for fast access
```
