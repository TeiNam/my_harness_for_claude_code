...
```swift
// PASS: Good: Check authorization before starting
guard ActivityAuthorizationInfo().areActivitiesEnabled else {
...
// FAIL: Bad: Start without checking authorization
let activity = try Activity.request(attributes: attrs, content: content, pushType: .token)
```
...
```swift
// PASS: Good: Continuously observe token changes
for await token in activity.pushTokenUpdates {
    await sendTokenToServer(token)
}

// FAIL: Bad: Read token once and assume it stays the same
...
```swift
// PASS: Good: Provide final state when ending
await activity.end(
    ActivityContent(state: finalState, staleDate: nil),
    dismissalPolicy: .after(Date().addingTimeInterval(4 * 3600))
)

// FAIL: Bad: End with nil content, leaving stale data displayed
...
```swift
// PASS: Good: Keep ContentState small and Codable
struct ContentState: Codable, Hashable {
    var score: Int
...
// FAIL: Bad: Large or complex data in ContentState
struct ContentState: Codable, Hashable {
...
```swift
// PASS: Good: Set a stale date so the system can indicate outdated content
let content = ActivityContent(
    state: state,
    staleDate: Date().addingTimeInterval(15 * 60)
)

// FAIL: Bad: No stale date; user sees outdated data with no indication
