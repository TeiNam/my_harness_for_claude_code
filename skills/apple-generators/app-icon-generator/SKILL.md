...
- **Style Modifiers**: stroke weight (WARNING: skip Shine/shadow for iOS 26+ targets — the system applies Liquid Glass specular and shadow; baking them in double-exposes)
...
| `drawShine` | Elliptical specular highlight | WARNING: Legacy only — off for iOS 26+ (system paints its own specular) |
| `drawShadow` | Drop shadow beneath element | WARNING: Legacy only — off for iOS 26+ (system casts its own shadow) |
...
WARNING: For an iOS 26+ / macOS 26+ target this is a PLACEHOLDER. Finish the real
...
- "Add more depth" → add a radial glow (WARNING: not baked shine on iOS 26+ — add depth in Icon Composer instead)
