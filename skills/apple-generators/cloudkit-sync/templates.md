...
```swift
// FAIL: Wrong -- old pattern, replaced by CKSyncEngine
...
```swift
// FAIL: Wrong -- creates a new record without preserving change tag
let record = CKRecord(recordType: "Item", recordID: recordID)
record["title"] = "Updated"
// This will always conflict with the server version

// PASS: Right -- restore system fields from last fetch
...
```swift
// FAIL: Wrong -- manual polling
Timer.scheduledTimer(withTimeInterval: 30, repeats: true) { _ in
    self.fetchChanges()
}

// PASS: Right -- CKSyncEngine handles change fetching via push notifications
...
```swift
// FAIL: Wrong -- waiting for sync before showing UI
func viewDidLoad() {
    await syncEngine.syncAllData()  // Blocks UI
    loadItems()
}

// PASS: Right -- show local data immediately, sync in background
func viewDidLoad() {
...
```swift
// FAIL: Wrong -- container ID scattered across files
...
// PASS: Right -- single source of truth
