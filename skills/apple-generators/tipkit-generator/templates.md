...
```swift
// PASS: Good: One tip per file, clear naming, meaningful image
struct SearchTip: Tip {
...
```swift
// FAIL: Bad: Multiple tips crammed into one file with vague names
struct Tips {
    struct Tip1: Tip { /* ... */ }
    struct Tip2: Tip { /* ... */ }
    struct Tip3: Tip { /* ... */ }
...
```swift
// PASS: Good: Configure early in the app lifecycle
@main
struct MyApp: App {
    var body: some Scene {
        WindowGroup {
...
```swift
// FAIL: Bad: Configure inside a deeply nested view (may run too late or multiple times)
...
```swift
// PASS: Good: Use .daily or .weekly for production to avoid overwhelming users
.displayFrequency(.daily)
```

```swift
// FAIL: Bad: Using .immediate in production -- users see every tip at once
.displayFrequency(.immediate)
```
...
```swift
// PASS: Good: Invalidate the tip instance that is currently displayed
struct SearchView: View {
    let searchTip = SearchTip()
...
```swift
// FAIL: Bad: Creating a new instance to invalidate (does not affect the displayed tip)
Button("Search") {
    performSearch()
...
```swift
// PASS: Good: Update parameter at the point where the condition becomes true
...
```swift
// FAIL: Bad: Setting the parameter in the tip's own init (defeats the purpose of rules)
struct FilterTip: Tip {
...
```swift
// PASS: Good: Donate events at natural user interaction points
List(items) { item in
    ItemRow(item: item)
}
.onAppear {
...
```swift
// FAIL: Bad: Donating events in a timer or background task (inflates counts artificially)
...
```swift
// PASS: Good: Use TipGroup when tips should appear in a logical learning sequence
...
```swift
// FAIL: Bad: Manually tracking tip ordering with state variables
