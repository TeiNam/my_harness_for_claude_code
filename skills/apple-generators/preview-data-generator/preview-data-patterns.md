...
- PASS: Always wrap preview data in `#if DEBUG`
- PASS: Reuse `.fixture()` if it exists; otherwise keep `.preview` self-contained
- PASS: Use stable seed strings for image URLs (e.g. `picsum.photos/seed/...`) so previews don't flicker
- PASS: For SwiftData, always `isStoredInMemoryOnly: true` — never seed the real store
- FAIL: Don't hit the live network in a preview — pin the ViewModel/service to a state
- FAIL: Don't ship preview data — `#if DEBUG` is non-negotiable
