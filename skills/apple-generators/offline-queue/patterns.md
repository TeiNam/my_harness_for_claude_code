...
```swift
// FAIL: Don't queue reads — cache them instead
if !networkMonitor.isConnected {
    queueManager.enqueue(OfflineOperation(endpoint: "/api/feed", httpMethod: .get))
}

// PASS: Use HTTP cache or local store for reads
if !networkMonitor.isConnected {
...
```swift
// FAIL: Queue a 100 MB video upload as a normal operation
let operation = OfflineOperation(
...
// PASS: Save large files to disk, queue a reference
...
```swift
// FAIL: Retry a payment without idempotency key
// Could charge the user twice!
try await apiClient.post("/api/payments", body: paymentData)

// PASS: Always include idempotency key for mutations
...
```swift
// FAIL: Wifi drops and reconnects rapidly → queue processes 10 times in 30 seconds
...
// PASS: Debounce connectivity changes
...
```swift
// FAIL: Token may expire by the time operation executes
let operation = OfflineOperation(
    endpoint: "/api/posts",
    headers: ["Authorization": "Bearer \(currentToken)"]  // Stale in 1 hour
)

// PASS: Inject fresh token at execution time
func execute(_ operation: OfflineOperation) async throws -> OfflineExecutionResult {
