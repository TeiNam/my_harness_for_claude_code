...
```swift
// FAIL: Wrong — "Accept" is prominent, "Reject" is a text link
...
// PASS: Right — Both actions have clear, tappable buttons
Button("Accept All") { acceptAll() }
    .buttonStyle(.borderedProminent)
    .controlSize(.large)
...
```swift
// FAIL: Wrong — Re-showing consent banner every launch after rejection
.onAppear {
    showConsentBanner = true // Always shows, even after explicit rejection
}

// PASS: Right — Only show when not yet determined
.onAppear {
...
### Ignoring ATT Denial
```swift
// FAIL: Wrong — Tracking despite ATT denial
func trackEvent(_ event: Event) {
    // No ATT check — Apple will reject this
    analyticsSDK.track(event)
}

// PASS: Right — Check both ATT and app consent
func trackEvent(_ event: Event) {
    #if os(iOS)
...
```swift
// FAIL: Wrong — Only stores current state, no history
UserDefaults.standard.set(true, forKey: "analyticsConsent")

// PASS: Right — Records decision with full audit metadata
