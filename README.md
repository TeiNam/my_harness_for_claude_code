...
에이전트마다 작업 성격에 맞는 모델 티어를 `model:` frontmatter에 **별칭**(`opus` / `sonnet` / `haiku`)으로 선언합니다. 버전 ID를 직접 박지 않으므로, 모델이 업그레이드돼도 대규모 재태깅 없이 최신 라인업을 따라갑니다. (`fable` 별칭도 유효하지만 frontmatter 에는 쓰지 않습니다 — 아래 표 참조.)
