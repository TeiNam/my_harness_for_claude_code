...
![Tests](https://img.shields.io/badge/tests-1554%20passing-brightgreen.svg)
...
특히 **AWS Bedrock 기반 개발을 집중 지원합니다**. `aws-bedrock` 스킬이 Converse API, 모델 호출(Claude·Nova·Llama·Mistral·Titan), Agents·Knowledge Bases, Guardrails, prompt caching, 크로스 리전 inference profile, 비용 추적을 다룹니다. 여기에 `aws-cloud`(IAM·네트워킹·비용 가드레일), `cost-aware-llm-pipeline`(토큰·비용 최적화), `claude-api`(Anthropic SDK 연계)가 붙어 AWS 경계 안의 LLM 개발을 일관되게 지원합니다.
...
**티어를 가르는 질문은 "박스가 열려 있는가"입니다.** 추론 깊이는 답의 형태가 미정일 때만 의미가 있습니다. taxonomy·rubric 이 이미 있거나 원인과 해결이 확정된 상태에서 실행만 하는 일은 탐색 공간이 없으므로 Sonnet 5 로 충분합니다. **여기에 opus 를 붙이는 것이 에이전트 함대에서 가장 흔한 낭비였습니다.** 반대로 설계·미지의 원인 진단·의미 보존 판정처럼 답을 스스로 만들어야 하는 일은 Opus 5 를 씁니다. Opus 5가 천장이므로 그 위로는 **티어가 아니라 effort**(`high`→`xhigh`→`max`)를 올리고, 그다음은 위가 아니라 **옆(Codex, 다른 모델 패밀리)**으로 갑니다.
...
- `code-reviewer`, `python-reviewer`, `typescript-reviewer`, `rust-reviewer`, `architect` — 언어별 리뷰어와 아키텍트 조합
...
- **`aws-bedrock` 스킬** — Converse API, `bedrock-runtime` 모델 호출(Claude / Nova / Llama / Mistral / Cohere / Titan), Bedrock Agents·Knowledge Bases(관리형 RAG). 그리고 Guardrails(PII·금칙어·문맥 필터), Titan/Cohere 임베딩, prompt caching, provisioned throughput, 크로스 리전 inference profile. Bedrock vs 직접 provider SDK 선택 기준표 포함.
...
워크로드 설치 시 `--category=ai,cloud` (또는 `--workload=ai,cloud`) 를 고르면 위 자산만 골라 설치합니다. 관련 트리거 키워드: `bedrock-runtime`, `Converse`, `InvokeModel`, `BedrockAgent`, `retrieve_and_generate`, `guardrail`, `inference profile`, `provisioned throughput`.

## 설치

전제: Node.js >=18, Python 3.12(`.tool-versions` 핀), MCP 를 쓰려면 Docker.

설치는 **도메인 축 6개 톱레벨 카테고리**(**개발 / 클라우드·인프라 / AI / 데이터 / 리서치·리포트 / 글쓰기**) 에서 시작합니다. 각 카테고리에서 중분류(sub-옵션: 언어·엔진·영역) 를 다중 선택하고, 자산이 많은 중분류(writing.social)만 상세 3그룹으로 한 단계 더 세분화합니다. 선택한 sub-옵션을 워크로드 키로 변환해, 그 키와 교집합인 자산만 `~/.claude/` 에 파일별 심볼릭 링크로 설치합니다. 저장소에서 수정한 내용은 즉시 반영됩니다.
...
방향키 체크박스로 고릅니다: `↑`/`↓` 이동, `space` 토글, `a` 전체, `enter` 확정. 흐름은 3단계입니다 — **대분류(카테고리) → 중분류(sub-옵션) → 상세(있을 때만)**. 상세 tier 는 자산이 많은 카테고리(**글쓰기 › 소셜** = 보이스/콘텐츠/시각)에만 나타나며, 나머지는 중분류에서 바로 끝납니다. 각 단계에서 아무것도 안 고르면 그 단계 전체를 고른 것으로 처리합니다.

설치는 먼저 **글로벌 baseline 상태**(`absent`/`outdated`/`current`)를 검사해 알려줍니다 — 매니페스트(`~/.claude/_harness-manifest.json`)의 버전을 저장소 `VERSION` 과 비교하며, 없거나 오래됐으면 새로 깔고 최신이면 선택 워크로드만 반영합니다.
...
> sub-옵션 플래그(`--dev=...` 등)를 명시하면 해당 카테고리를 자동으로 활성화하므로 `--category=` 는 생략해도 됩니다. 상세 플래그(`--writing-social=`)도 마찬가지입니다.
...
선택한 워크로드(agent·command·skill·rule)를 설치하고, **워크로드 밖인
hooks·mcp 는 설치 후 대화형 터미널(TTY)일 때 추가 설치할지 물어봅니다.** `--with-hooks`
를 지정하면 hooks 를 묻지 않고 바로 병합하고, `--with-mcp` 를 지정하면 MCP proxy 를 묻지 않고
...
`engines.node`는 `>=18`. ESLint 9.x로 lint를 실행합니다.
...
- 프록시 에셋: `mcp-configs/proxy/` (`docker-compose.yaml` · `config.json` · `.env.example`). `install.sh` 가 설치 중 물어보고(또는 `--with-mcp` 로 바로) `docker compose up -d` 로 기동합니다. **docker 가 없으면** 설치 명령을 안내하고 넘어갑니다. brew 가 있으면 `brew install colima docker docker-compose && colima start`, 없으면 Colima/Docker Desktop 링크를 띄우고, 재실행 커맨드는 `./install.sh --with-mcp` 입니다. 데몬 미동작·compose v2 부재도 각각 켜는 법을 안내합니다.
- 활성 클라이언트 설정: `.mcp.json` — 프록시 서버는 `http://localhost:9090/<서버>/mcp` URL 로, 로컬 서버(sentry·playwright)는 직접 명령으로 적습니다. 현재 이 저장소의 `.mcp.json` 에는 `context7` 하나만 선언돼 있고, 프록시가 실제로 띄우는 나머지 서버(`proxy/config.json` 기준 9개)는 필요할 때 같은 형식으로 추가합니다.
...
- 리뷰: `/code-review`, `/cross-review`(codex 교차 모델), `/python-review`, `/rust-review`, `/fastapi-review`
...
전체 목록과 각 커맨드가 참조하는 에이전트·스킬은 `docs/COMMAND-REGISTRY.json`에 있습니다.

## 검증과 테스트

```bash
npm run lint                      # ESLint
npm test                          # CI 검증 + 전체 테스트 슈트(1554개)
...
4. `validate-no-personal-paths` — 배포 대상 경로에 개인 절대 경로가 없는지 확인
...
- 파일 이름은 소문자 + 하이픈
- 훅 스크립트는 작게 유지하고 단일 책임만 지며, 비치명적 오류 시 항상 exit 0으로 종료해 도구 호출을 막지 않음
...
- `docs/rules-reference/README.md` — 규칙 레이어 구조와 프로젝트별 설치
