...
설치는 7개 톱레벨 카테고리(**backend / frontend / plugin / data-analysis / data-design / writing / apple**) 와 sub-옵션·상세로 결정되는 **3단계(대분류→중분류→상세)** 메뉴다. sub-옵션이 곧 워크로드 키와 1:1 매칭되어, 예컨대 "데이터 설계 → MySQL" 만 골랐을 때 Postgres 가이드까지 끌려오지 않는다. "글쓰기" 카테고리는 sub-옵션 2개(**기술 문서** → `writing` / **소셜 콘텐츠**)로 나뉘고, 소셜은 다시 상세 3그룹(`social-voice`/`social-content`/`social-visual`)으로 드릴다운된다. "Apple 플랫폼 개발"은 sub-옵션 없이 카테고리 레벨에서 곧장 상세 3그룹(`apple-core`/`apple-platform`/`apple-product`)으로 내려간다 — owner의 기존 6개 워크로드와 무관한 opt-in 전용 카테고리. 상세 tier 는 자산이 많은 apple(23)·social(17) 두 곳에만 붙였고, 나머지 중분류는 leaf(3단계 미진입)로 유지된다.
...
- 워크로드 키 카탈로그는 `scripts/install/workloads.js` (`core, python-backend, python-data, rust, nodejs, cloud, devops, finops, data-analysis, ai, frontend, obsidian, plugin-chrome, plugin-claude, mysql, postgres, mongodb, dynamodb, writing, social-voice, social-content, social-visual, apple-core, apple-platform, apple-product`). `core` 는 항상 포함된다. `devops`(AWS IaC·컨테이너·서버리스·관측성)·`finops`(비용·요금)·`data-analysis`(AWS Glue·Athena·Redshift; 로컬 pandas/duckdb 는 `python-data`) 는 AWS MCP 서버 분류용으로, `cloud` 가 통짜 바구니가 되는 걸 막는다. 옛 통짜 키 `apple` 은 `workloads.js` 의 `expandAliases()` 가 `apple-core,apple-platform,apple-product` 로 확장하는 별칭으로 계속 동작한다(`--workload=apple`, `--category=apple`). `lab` 은 메뉴에 노출되지 않는 수동 전용 키로 (`--workload=...,lab`), humanize 메타 에이전트 격리에만 쓴다.
