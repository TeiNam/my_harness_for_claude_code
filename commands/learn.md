...
Analyze the current session, extract patterns worth keeping, and save them as
skill files — with a quality gate, save-location decision, and
knowledge-placement awareness before writing anything.
...
The quality gate uses a checklist-based holistic verdict system rather than a
numeric scoring rubric. Modern frontier models have strong contextual judgment —
forcing rich qualitative signals into numeric scores loses nuance and can
produce misleading totals. The holistic approach lets the model weigh all
factors naturally, while the explicit checklist ensures no critical check is
skipped.
