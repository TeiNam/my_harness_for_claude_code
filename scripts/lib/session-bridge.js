...
/**
 * Shared session bridge utilities for harness hooks.
 *
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `harness-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `harness-metrics-bridge` and the background
 * `harness-statusline`, both writing to the same session bridge) do not
 ...
 *
 * Same pattern used by `writeCostWarningIfChanged` in
 * `scripts/hooks/harness-metrics-bridge.js` for the cost-warning
 * cache; this primitive applies it to the session bridge too.
 *
 * @param {string} sessionId - Already-sanitized session ID
 * @param {object} data
 */
...
function resolveSessionId() {
  const raw = process.env.HARNESS_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
