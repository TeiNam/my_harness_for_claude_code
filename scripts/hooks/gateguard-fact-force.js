...
const GATEGUARD_DISABLE_VALUES = new Set(['0', 'false', 'off', 'disabled', 'disable']);
...
function isGateGuardDisabled() {
  ...
  const raw =
    process.env.HARNESS_GATEGUARD !== undefined
      ? process.env.HARNESS_GATEGUARD
      : process.env.HARNESS_GATEGUARD;
  return GATEGUARD_DISABLE_VALUES.has(normalizeEnvValue(raw));
}
...
function resolveSessionKey(data) {
  const directCandidates = [data && data.session_id, data && data.sessionId, data && data.session && data.session.id, process.env.CLAUDE_SESSION_ID, process.env.HARNESS_SESSION_ID];
...
function withRecoveryHint(message, hookIds = [EDIT_WRITE_HOOK_ID]) {
  ...
  return [
    message,
    '',
    `Recovery: if GateGuard is blocking setup or repair work, run this session with \`HARNESS_GATEGUARD=off\` or add ${disableTargets} to \`HARNESS_DISABLED_HOOKS\`.`
  ].join('\n');
}
