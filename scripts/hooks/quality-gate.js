 ...
 * stop-format-typecheck.js already runs `biome check --write`.
 ...
 * Skips JS/TS files when Biome is configured (handled by stop-format-typecheck).
 *
...
function maybeRunQualityGate(filePath) {
  ...
  const fix = String(process.env.HARNESS_QUALITY_GATE_FIX || '').toLowerCase() === 'true';
  const strict = String(process.env.HARNESS_QUALITY_GATE_STRICT || '').toLowerCase() === 'true';
  ...
  if (['.ts', '.tsx', '.js', '.jsx', '.json', '.md'].includes(ext)) {
    ...
    if (formatter === 'biome') {
      // JS/TS already handled by stop-format-typecheck via `biome check --write`
