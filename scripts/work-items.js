...
Track Linear, GitHub, handoff, and manual roadmap items in the harness SQLite
state store so the status command can include linked work and blocked follow-up.

Options:
  --id <id>                 Stable local work-item id for upsert
  --source <source>         Source system, e.g. linear, github, handoff, manual
  --source-id <id>          Source-local identifier, e.g. PR-42 or ticket id
  ...
  --session-id <id>         Optional harness session id
...
function runGhJson(args) {
  const shimPath = process.env.HARNESS_GH_SHIM;
...
function buildGithubPrWorkItem(repo, pr, options = {}) {
  return {
    ...
    metadata: {
      ...
      syncedBy: 'harness-work-items-sync-github',
    },
  };
}
...
function closeStaleGithubItems(store, repo, activeIds, options = {}) {
  ...
  for (const item of payload.items) {
    if (!item.metadata || item.metadata.syncedBy !== 'harness-work-items-sync-github') {
      continue;
    }
