...
function runTests() {
  ...
  if (
    test('returns path containing harness-metrics-', () => {
      const p = getBridgePath('test-session');
      assert.ok(p.includes('harness-metrics-'), `Expected harness-metrics- in path, got: ${p}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveSessionId uses HARNESS_SESSION_ID env var', () => {
      const original = process.env.HARNESS_SESSION_ID;
      try {
        process.env.HARNESS_SESSION_ID = 'env-session-42';
        ...
        if (original === undefined) {
          ...
          process.env.HARNESS_SESSION_ID = original;
        }
      }
    })
  )
