/**
 * Tests for the SQLite-backed harness state store and CLI commands.
 */
...
const mode = process.env.HARNESS_FAKE_GH_MODE || 'open';
...
async function seedStore(dbPath) {
  ...
  store.upsertSession({
    ...
    harness: 'claude',
    state: 'active',
    repoRoot: '/tmp/harness-repo',
    startedAt: '2026-03-15T08:00:00.000Z',
    endedAt: null,
    snapshot: {
      schemaVersion: 'harness.session.v1',
      adapterId: 'dmux-tmux',
      session: {
        id: 'session-active',
        kind: 'orchestrated',
      ...
      workers: [
        {
          ...
          worktree: '/tmp/harness-repo/.worktrees/worker-1',
        },
        {
          id: 'worker-2',
          label: 'Worker 2',
          state: 'idle',
          branch: 'feat/state-store',
          worktree: '/tmp/harness-repo/.worktrees/worker-2',
        },
      ],
      aggregates: {
        workerCount: 2,
  ...
  store.insertSkillRun({
    ...
    skillVersion: '1.0.0',
    sessionId: 'session-recorded',
    taskDescription: 'Outline harness 2.0 work',
    outcome: 'unknown',
    failureReason: null,
    tokensUsed: 300,
    durationMs: 500,
  ...
  store.insertDecision({
    ...
    rationale: 'Need queryable local state for harness control plane',
...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    const homeDir = createTempDir('harness-state-home-');

    try {
      const expectedPath = path.join(homeDir, '.claude', 'harness', 'state.db');
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    const tempDir = createTempDir('harness-state-memory-');
  ...
  if (await test('stores sessions and returns detailed session views with workers, skill runs, and decisions', async () => {
    const testDir = createTempDir('harness-state-db-');
  ...
  if (await test('builds an empty status snapshot with null rates and missing install health', async () => {
    const testDir = createTempDir('harness-state-empty-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('tracks linked work items for Linear, GitHub, and handoff progress', async () => {
    const testDir = createTempDir('harness-state-work-items-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const linearItem = store.upsertWorkItem({
        id: 'linear-hrn-20-control-plane',
        source: 'linear',
        sourceId: 'HRN-20',
        title: 'Define harness-neutral session/worktree contract',
        status: 'in-progress',
        priority: 'high',
        url: 'https://linear.app/harnesstools/issue/HRN-20',
        owner: 'control-plane',
        repoRoot: '/tmp/harness-repo',
        sessionId: 'session-active',
        metadata: {
          project: 'Harness 2.0: Control Plane',
        },
      ...
      store.upsertWorkItem({
        ...
        source: 'handoff',
        sourceId: 'harness-rc1-release-decision-20260511.md',
      ...
      assert.strictEqual(linearItem.id, 'linear-hrn-20-control-plane');
      assert.strictEqual(linearItem.metadata.project, 'Harness 2.0: Control Plane');
  ...
  if (await test('normalizes default optional fields and reports warning install health', async () => {
    const testDir = createTempDir('harness-state-defaults-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('rejects invalid limits and unserializable JSON payloads', async () => {
    const testDir = createTempDir('harness-state-errors-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI supports human-readable and --json output', async () => {
    const testDir = createTempDir('harness-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI --exit-code reports attention without suppressing output', async () => {
    const attentionDir = createTempDir('harness-state-attention-');
    const okDir = createTempDir('harness-state-ok-');
  ...
  if (await test('status CLI can emit and write markdown operator snapshots', async () => {
    ...
    try {
      ...
      assert.match(written, /^# Harness Status/m);
  ...
  if (await test('work-items CLI supports upsert, list, show, and close', async () => {
    const testDir = createTempDir('harness-work-items-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      const upsertResult = runNode(WORK_ITEMS_SCRIPT, [
        ...
        'high',
        '--url',
        'https://linear.app/example/issue/HRN-99',
        '--owner',
        'control-plane',
        '--metadata-json',
        '{"project":"Harness 2.0"}',
      ...
      assert.strictEqual(listPayload.items[0].id, 'linear-hrn-99');

      const showResult = runNode(WORK_ITEMS_SCRIPT, ['show', 'linear-hrn-99', '--db', dbPath]);
      assert.strictEqual(showResult.status, 0, showResult.stderr);
      assert.match(showResult.stdout, /linear\/#HRN-99 in-progress: Ship work item CLI/);

      const closeResult = runNode(WORK_ITEMS_SCRIPT, ['close', 'linear-hrn-99', '--db', dbPath, '--json']);
  ...
  if (await test('work-items CLI syncs GitHub PRs and issues into readiness', async () => {
    const testDir = createTempDir('harness-work-items-github-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      const env = {
        HARNESS_GH_SHIM: createGhShim(binDir),
      };
