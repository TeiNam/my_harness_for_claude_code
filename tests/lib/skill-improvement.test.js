...
test('observation layer writes and reads structured skill outcomes', () => {
  const projectRoot = makeProjectRoot('harness-skill-observe-');
  ...
  try {
    ...
    assert.strictEqual(records[0].schemaVersion, 'harness.skill-observation.v1');
    ...
    assert.strictEqual(getSkillObservationsPath({ projectRoot }), path.join(projectRoot, '.claude', 'harness', 'skills', 'observations.jsonl'));
  } finally {
    cleanup(projectRoot);
  }
});
...
test('health inspector traces recurring failures for a skill across runs', () => {
  const projectRoot = makeProjectRoot('harness-skill-health-');
...
test('amendify proposes SKILL.md patch content from failure evidence', () => {
  ...
  assert.strictEqual(proposal.schemaVersion, 'harness.skill-amendment-proposal.v1');
...
test('evaluation scaffold compares amended and baseline performance', () => {
  ...
  assert.strictEqual(evaluation.schemaVersion, 'harness.skill-evaluation.v1');
