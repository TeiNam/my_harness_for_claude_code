...
function runScript(args = [], envOverrides = {}) {
  ...
  if (!envOverrides.HARNESS_HOOK_PROFILE) delete env.HARNESS_HOOK_PROFILE;
  if (!envOverrides.HARNESS_DISABLED_HOOKS) delete env.HARNESS_DISABLED_HOOKS;
...
function runTests() {
  ...
  if (test('returns yes for strict profile with strict-only hook', () => {
    const result = runScript(['my-hook', 'strict'], { HARNESS_HOOK_PROFILE: 'strict' });
    assert.strictEqual(result.stdout, 'yes');
  })) passed++; else failed++;

  if (test('returns yes for minimal profile with minimal-only hook', () => {
    const result = runScript(['my-hook', 'minimal'], { HARNESS_HOOK_PROFILE: 'minimal' });
    assert.strictEqual(result.stdout, 'yes');
  })) passed++; else failed++;
